...
43.6591° N, 70.2568° W respectively. No population is added, these are solely treated as sensors to increase accuracy for nearby
cities.
'''
import pandas as pd
...
def add_sensors_only(temp_readings, name, temp_row, lon, lat):
    
    temp_readings.append({"Date":temp_row["location_date"], "City/State":name, "Weighted Pop":0, "Lon":lon, "Lat":lat, "Mean":temp_row["temp_mean_c"],  
                      "Min":temp_row["temp_min_c"], "Max":temp_row["temp_max_c"]})
...
for index, temp_row in temp_df.iterrows():
    ...
    if (name == "Albany"):
        temp_readings = add_sensors_only(temp_readings, "Albany/New York", temp_row, 42.6526, 73.7562)
        continue
    ...
    if (name == "Portland"):
        if (temp_row["station_code"] == "KPDX"):
            ...
            temp_readings = add_sensors_only(temp_readings, "Portland/Maine", temp_row, 43.6591, 70.2568)
            continue
...
new_df.to_csv("temp_all_cities_added_lat_lon.csv", index=False)
